...
/**
 ** \file /home/ferhat/git/vc-gen/gc/recognizer/gc-parser.hpp
 ** Define the gc::parser class.
 */
...
# if 201103L <= YY_CPLUSPLUS
  ...
  private:
    ...
    {
      // assertion
      char dummy1[sizeof (gc::ast::Assertion*)];

      // command
      char dummy2[sizeof (gc::ast::Command*)];

      // comparison
      char dummy3[sizeof (gc::ast::Comparison*)];

      // constant
      char dummy4[sizeof (gc::ast::Constant*)];

      // expression
      char dummy5[sizeof (gc::ast::Expression*)];

      // location
      char dummy6[sizeof (gc::ast::Location*)];

      // prog
      char dummy7[sizeof (gc::ast::Program*)];
      ...
      // commands
      char dummy10[sizeof (std::vector<gc::ast::Command*>)];
...
#endif
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, gc::ast::Command*&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const gc::ast::Command*& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
#endif
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, gc::ast::Expression*&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const gc::ast::Expression*& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
#endif
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, std::vector<gc::ast::Command*>&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const std::vector<gc::ast::Command*>& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
switch (yytype)
    ...
    gc_parser (gc_driver& driver_yyarg);
    virtual ~gc_parser ();
  ...
  private:
    ...
    gc_parser (const gc_parser&);
    gc_parser& operator= (const gc_parser&);
  ...
  inline
  gc_parser::token_number_type
  gc_parser::yytranslate_ (token_type t)
  {
...
#if 201103L <= YY_CPLUSPLUS
  template <typename Base>
  gc_parser::basic_symbol<Base>::basic_symbol (basic_symbol&& that)
    : Base (std::move (that))
    , value ()
  ...
  template <typename Base>
  gc_parser::basic_symbol<Base>::basic_symbol (const basic_symbol& that)
    : Base (that)
    , value ()
  ...
  template <typename Base>
  bool
  gc_parser::basic_symbol<Base>::empty () const YY_NOEXCEPT
  {
  ...
  template <typename Base>
  void
  gc_parser::basic_symbol<Base>::move (basic_symbol& s)
  {
    super_type::move (s);
    switch (this->type_get ())
    {
  ...
  // by_type.
  inline
  gc_parser::by_type::by_type ()
    : type (empty_symbol)
  {}

#if 201103L <= YY_CPLUSPLUS
  inline
  gc_parser::by_type::by_type (by_type&& that)
    : type (that.type)
  {
    that.clear ();
  }
  ...
  inline
  gc_parser::by_type::by_type (const by_type& that)
    : type (that.type)
  {}

  inline
  gc_parser::by_type::by_type (token_type t)
    : type (yytranslate_ (t))
  {}

  inline
  void
  gc_parser::by_type::clear ()
  {
    type = empty_symbol;
  }
  ...
  inline
  void
  gc_parser::by_type::move (by_type& that)
  {
    type = that.type;
    that.clear ();
  }
  ...
  inline
  int
  gc_parser::by_type::type_get () const YY_NOEXCEPT
  {
    return type;
  }

  inline
  gc_parser::token_type
  gc_parser::by_type::token () const YY_NOEXCEPT
  {

...
void when(char* date, char* hour) {
    ...
    now = time(nullptr);                   // Get date and time from system.
...
char* oclock(char* hour) {
    time_t now;      // Stores an integer encoding of date and time.
    char* nowstring; // Stores date and time in a readable form.
